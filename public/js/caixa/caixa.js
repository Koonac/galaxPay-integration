$(function () {
    ...
    var valorTotal = parseFloat($('#valorCaixaAbertura').val().replace(/[.]/g, "").replace(/[,]/g, "."));
    ...
    $('.valorRecebimento').each(function (i, valorRecebimento) {
        var valorItem = parseFloat($(valorRecebimento).val().replace(/[.]/g, "").replace(/[,]/g, "."));
    ...
    $('.valorDespesa').each(function (i, valorDespesa) {
        var valorItem = parseFloat($(valorDespesa).val().replace(/[.]/g, "").replace(/[,]/g, "."));
        if (!isNaN(valorItem)) {
            valorTotal -= valorItem;
        }
    })
